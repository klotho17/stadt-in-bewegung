export default function ProjectIcon({ width = 300, height = 100 }) {
  const gap = width * 0.03;

  const topRowHeight = height * 0.55;
  const bottomRowHeight = height - topRowHeight - gap;

  const topLeftWidth = width * 0.55;
  const topRightWidth = width - topLeftWidth - 3 * gap;

  const bottomLeftWidth = width * 0.38;
  const bottomMiddleWidth = width * 0.34;
  const bottomRightWidth = width - bottomLeftWidth - bottomMiddleWidth - 4 * gap;

  // Play button dimensions
  const playRadius = height * 0.3535; // Half of (height / √2)
  const cx = width / 2;
  const cy = height / 2;

  // Triangle dimensions
  const triangleWidth = playRadius * 0.9;
  const triangleHeight = playRadius * 1.2;

  const shiftX = playRadius * 0.1; // small rightward shift

  const trianglePoints = `
  ${cx - triangleWidth / 2 + shiftX},${cy - triangleHeight / 2}
  ${cx - triangleWidth / 2 + shiftX},${cy + triangleHeight / 2}
  ${cx + triangleWidth / 2 + shiftX},${cy}
`;


  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={width}
      height={height}
      viewBox={`0 0 ${width} ${height}`}
      role="img"
      aria-label="Drawing-based layout with rectangles and play button"
    >
      {/* Top Left */}
      <rect
        x={gap}
        y={gap}
        width={topLeftWidth}
        height={topRowHeight}
        fill="white"
        fillOpacity="1"
      />

      {/* Top Right */}
      <rect
        x={gap * 2 + topLeftWidth}
        y={gap}
        width={topRightWidth}
        height={topRowHeight}
        fill="white"
        fillOpacity="0.6"
      />

      {/* Bottom Left */}
      <rect
        x={gap}
        y={topRowHeight + 2 * gap}
        width={bottomLeftWidth}
        height={bottomRowHeight}
        fill="white"
        fillOpacity="0.4"
      />

      {/* Bottom Middle */}
      <rect
        x={gap * 2 + bottomLeftWidth}
        y={topRowHeight + 2 * gap}
        width={bottomMiddleWidth}
        height={bottomRowHeight}
        fill="white"
        fillOpacity="0.8"
      />

      {/* Bottom Right */}
      <rect
        x={gap * 3 + bottomLeftWidth + bottomMiddleWidth}
        y={topRowHeight + 2 * gap}
        width={bottomRightWidth}
        height={bottomRowHeight}
        fill="white"
        fillOpacity="0.3"
      />

      {/* Play Button Circle */}
      <circle
        cx={cx}
        cy={cy}
        r={playRadius}
        fill="black"
      />

      {/* Play Triangle */}
      <polygon
        points={trianglePoints}
        fill="white"
      />
    </svg>
  );
}
